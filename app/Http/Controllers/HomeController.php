...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        if(Auth::user()->role_id > 2 && $general_setting->staff_access == 'own') {
            ...
            $products = Product::whereDate('created_at', '>=' , $start_date)->where('user_id', Auth::id())->where('is_active', true)->whereDate('created_at', '<=' , $end_date)->sum('qty');
            $vehicles = VehicleInfo::whereDate('created_at', '>=' , $start_date)->where('user_id', Auth::id())->where('is_active', true)->whereDate('created_at', '<=' , $end_date)->sum('qty');
            $raws = Vehicle::whereDate('created_at', '>=' , $start_date)->where('user_id', Auth::id())->where('is_active', true)->whereDate('created_at', '<=' , $end_date)->count();
            $manufactures = Manufacture::whereDate('created_at', '>=' , $start_date)->where('status',1)->where('user_id', Auth::id())->where('is_active', true)->whereDate('created_at', '<=' , $end_date)->count();
            $vehicleproducts = VehicleProduct::whereDate('created_at', '>=' , $start_date)->where('user_id', Auth::id())->where('is_active', true)->whereDate('created_at', '<=' , $end_date)->sum('qty');
            $vehiclesales = Sale::whereDate('created_at', '>=' , $start_date)->where('user_id', Auth::id())->where('type',2)->whereDate('created_at', '<=' , $end_date)->sum('grand_total');
            $productsales = Sale::whereDate('created_at', '>=' , $start_date)->where('user_id', Auth::id())->where('type',1)->whereDate('created_at', '<=' , $end_date)->sum('grand_total');
        } 
        else {
            $product_sale_data = Product_Sale::select(DB::raw('product_id, sum(qty) as sold_qty, sum(total) as sold_amount'))->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->groupBy('product_id')->get();
            ...
            $products = Product::whereDate('created_at', '>=' , $start_date)->where('is_active', true)->whereDate('created_at', '<=' , $end_date)->sum('qty');
            $vehicles = VehicleInfo::whereDate('created_at', '>=' , $start_date)->where('is_active', true)->whereDate('created_at', '<=' , $end_date)->sum('qty');
            $raws = Vehicle::whereDate('created_at', '>=' , $start_date)->where('is_active', true)->whereDate('created_at', '<=' , $end_date)->sum('qty');
            $manufactures = Manufacture::whereDate('created_at', '>=' , $start_date)->where('is_active', true)->whereDate('created_at', '<=' , $end_date)->sum('qty');
            $vehicleproducts = VehicleProduct::whereDate('created_at', '>=' , $start_date)->where('is_active', true)->whereDate('created_at', '<=' , $end_date)->sum('qty');
            $vehiclesales = Sale::whereDate('created_at', '>=' , $start_date)->where('type',2)->whereDate('created_at', '<=' , $end_date)->sum('grand_total');
            $productsales = Sale::whereDate('created_at', '>=' , $start_date)->where('type',1)->whereDate('created_at', '<=' , $end_date)->sum('grand_total');
        }
        ...
        while($start < $end)
        {
            ...
            if(Auth::user()->role_id > 2 && $general_setting->staff_access == 'own') {
                ...
                $sent_amount = DB::table('payments')->whereNotNull('sale_id')->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->where('user_id', Auth::id())->sum('amount');
                ...
                $sent_amount = DB::table('payments')->whereNotNull('sale_id')->whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->sum('amount');
        ...
        while($start < $end)
        {
            ...
            if(Auth::user()->role_id > 2 && $general_setting->staff_access == 'own') {
                $sale_amount = Sale::whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->where('user_id', Auth::id())->where('type',1)->sum('grand_total');
                $purchase_amount =  Sale::whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->where('type',2)->sum('grand_total');
            }
            else{ 
                $sale_amount = Sale::whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->where('type',1)->sum('grand_total');
                $purchase_amount =  Sale::whereDate('created_at', '>=' , $start_date)->whereDate('created_at', '<=' , $end_date)->where('type',2)->sum('grand_total');
            } 
        ...
        DB::reconnect();
        return view('index', compact('vehiclesales','productsales','vehicleproducts','manufactures','raws','vehicles','products', 'month', 'yearly_sale_amount', 'yearly_purchase_amount',  'best_selling_qty', 'yearly_best_selling_qty', 'yearly_best_selling_price', 'all_permission'));
    }
